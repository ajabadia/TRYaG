...
**Tecnología:** IA Generativa (Google Gemini 2.5 Flash).
...
* **¿Qué hace?** Antes de que el "Médico Asistente" (Sección 1) o el "Residente Senior" (Sección 6) respondan, el sistema busca en los PDFs de protocolos del hospital la información relevante y **aumenta el contexto** de la IA. Le entrega la "chuleta" con la respuesta correcta para que no tenga que inventarla.
* **¿Por qué IA Local (Búsqueda)?**
  * **Grounding (Anclaje):** Evita alucinaciones. La IA solo puede responder basándose en los documentos que le hemos dado (Protocolos de Triaje, Guías de Práctica Clínica).
...
### 6. El "Residente Senior" (Segunda Opinión)

**Tecnología:** IA de Razonamiento Avanzado (Google Gemini 2.0 Pro / Reasoning).

* **¿Qué hace?** Analiza casos complejos con un enfoque de "pensamiento lento" (Chain of Thought). No busca responder rápido, sino explorar hipótesis, diagnósticos diferenciales y detectar riesgos sutiles que pasarían desapercibidos en un triaje rápido.
* **¿Por qué un modelo diferente al de Triaje?**
  * Para el triaje necesitamos **velocidad** (Flash). Para diagnósticos difíciles necesitamos **profundidad** (Pro/Reasoning). Es la diferencia entre un médico de triaje (decisión rápida) y un internista (análisis profundo).
* **Ventaja:** Actúa como una red de seguridad para el médico, sugiriendo pruebas olvidadas o alertando de rarezas.

---

### 7. ¿Qué pasa si la IA falla? (Modo Contingencia)

**Tecnología:** Fallback "Graceful Degradation" (Degradación Elegante).

* **El Problema:** Internet se cae, los servidores de Google responden lento o la IA tiene un error. En un hospital, no podemos parar.
* **La Solución:** El sistema detecta automáticamente si la IA no está disponible.
  * **Cambio de Modo:** Pasa instantáneamente a **"Modo Manual"**.
  * **Funcionalidad:** Los campos automáticos se vuelven manuales. El cálculo de riesgo (PTR/NEWS2) sigue funcionando porque es código local (Python), no depende de la nube.
* **Filosofía:** "La IA es un copiloto, no el piloto". El sistema funciona al 100% sin IA, solo que es menos mágico y requiere más escritura manual.
...
| **Segunda Opinión**      | Razonamiento         | **Gemini Pro**       | Profundidad y análisis de casos complejos.|
...
| **Contingencia**         | Sin IA               | **Python Puro**      | Continuidad operativa sin internet.      |
