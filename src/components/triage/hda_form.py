...
def render_hda_form(reset_count: int, disabled: bool = False):
    ...
    with st.expander("🧐 Historia de la Enfermedad Actual (HDA - ALICIA)", expanded=False):
        ...
        with c_hda1:
            ...
            st.session_state.datos_paciente['hda_intensidad'] = st.text_input("📈 Intensidad", value=st.session_state.datos_paciente.get('hda_intensidad', ''), disabled=disabled, key=f"hda_int_{reset_count}", help="Escala 1-10 o descripción (leve, moderado, severo)")
        with c_hda2:
