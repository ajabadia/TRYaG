...
def render_sensory_history_form(reset_count: int, disabled: bool = False):
    ...
    with st.expander("👂 Déficits Sensoriales y Ayudas", expanded=False):
        ...
        with c_sen1:
            has_auditory = st.checkbox("Déficit Auditivo (Usa audífonos)", value=st.session_state.datos_paciente.get('sens_auditivo', False), disabled=disabled, key=f"sens_aud_{reset_count}")
            st.session_state.datos_paciente['sens_auditivo'] = has_auditory
            if has_auditory:
                st.session_state.datos_paciente['sens_auditivo_det'] = st.text_input("Detalles Auditivo", value=st.session_state.datos_paciente.get('sens_auditivo_det', ''), key=f"sens_aud_det_{reset_count}", disabled=disabled)

            has_visual = st.checkbox("Déficit Visual (Usa gafas/lentes)", value=st.session_state.datos_paciente.get('sens_visual', False), disabled=disabled, key=f"sens_vis_{reset_count}")
            st.session_state.datos_paciente['sens_visual'] = has_visual
            if has_visual:
                st.session_state.datos_paciente['sens_visual_det'] = st.text_input("Detalles Visual", value=st.session_state.datos_paciente.get('sens_visual_det', ''), key=f"sens_vis_det_{reset_count}", disabled=disabled)
