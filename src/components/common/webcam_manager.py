...
def render_webcam_manager(key_prefix="webcam", on_close=None, disabled=False):
    ...
    with c_save:
        # Botón Guardar y Cerrar (Imperativo)
        if st.button("✅ Guardar y Cerrar", key=f"{key_prefix}_save", type="primary", use_container_width=True, disabled=not photos or disabled):
